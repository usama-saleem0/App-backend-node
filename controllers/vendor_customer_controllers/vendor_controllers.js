...
const get_matching_job = async (req, res, next) => {
    ...
    try {
        ...
        const firstThreeDigits = datas.substring(0, 2)
                        ...
                        },
                        {
                            $match: {
                                $or: [
                                    ...
                                    { "jobs.zipcode": { $regex: `^${firstThreeDigits}` } },
...
// is se upaar wala zip code k saath matching kreigaaa neeche wala sirf queries pr filter kreiga..
...
const create_expert_gig = async (req, res,next) => {
    const { body } = req;
    try {
        const {
            vender_id,
